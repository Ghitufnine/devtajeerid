...
class OrderController extends Controller
{
    ...
    public function show(FoodOrderDataTable $foodOrderDataTable, $id)
    {
        ...
        $shops = \DB::table('orders')
                ...
                ->select('restaurants.name as name', 'restaurants.phone as phone','restaurants.address as pickup_address', 'restaurants.latitude as latitude', 'restaurants.longitude as longitude')
                ->where('orders.id', $id)
                ->groupBy('orders.id')
                ->first();
        $subtotal = 0;
        ...
        return $foodOrderDataTable->render('orders.show', ["order" => $order,"shops" => $shops , "total" => $total, "subtotal" => $subtotal]);
    }
