...
use App\Repositories\ShopCategoriesRepository;
use Flash;
...
class RestaurantController extends Controller
{
    ...
    /**
     * @var ShopCategoriesRepository
     */
    private $shopCatRepository;

    public function __construct(RestaurantRepository $restaurantRepo, CustomFieldRepository $customFieldRepo, UploadRepository $uploadRepo, UserRepository $userRepo, ShopCategoriesRepository $shopCatRepo)
    {
        parent::__construct();
        ...
        $this->shopCatRepository = $shopCatRepo;
    }
    ...
    public function create()
    {
        ...
        $usersSelected = [];
        $driversSelected = [];
        $category = $this->shopCatRepository->pluck('name', 'id');
    ...
    public function edit($id)
    {
        ...
        return view('restaurants.edit')->with('restaurant', $restaurant)->with("customFields", isset($html) ? $html : false)->with("user", $user)->with("drivers", $drivers)->with("usersSelected", $usersSelected)->with("driversSelected", $driversSelected)->with("category", $category);
    }
    ...
    {
        $shops = DB::table('user_restaurants')
                ->join('restaurants', 'restaurants.id', '=', 'user_restaurants.restaurant_id')
                ->join('users', 'users.id', '=', 'user_restaurants.user_id')
                ->select('users.name as user', 'restaurants.name as shop', 'restaurants.id as id')
                ->orderBy('restaurants.id', 'asc')
                ->get();
        $count = DB::table('user_restaurants')->count();
        $counts = DB::table('driver_restaurants')->count();  
        $drivers = DB::table('driver_restaurants')
                ->join('restaurants', 'restaurants.id', '=', 'driver_restaurants.restaurant_id')
                ->join('users', 'users.id', '=', 'driver_restaurants.user_id')
                ->select('users.name as driver', 'restaurants.name as shop', 'restaurants.id as id')
                ->orderBy('restaurants.id', 'asc')->get();  

        return view('shops', compact('shops', 'count', 'drivers', 'counts'));
    }
}
